		...
		while ($row = mysql_fetch_row($result))
			{
				...
				$last = $row[2];
				$dob = $row[3];
				echo "<a href='javascript:void(0);' onclick='window.opener.document.form1.director.value=\"$last, $first ($dob)\";window.opener.document.form1.did.value=\"$did\";self.close();'>$last, $first ($dob)</a> <br/>";
			}
	}
	mysql_close($db_connection);
?>
